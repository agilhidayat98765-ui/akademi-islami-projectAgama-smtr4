<x-app-layout>
    <div x-data="{ activeTab: 'dipelajari' }" class="max-w-5xl mx-auto space-y-8">
        ...
        <div x-show="activeTab === 'dipelajari'" x-cloak>
            ...
            @else
                <div class="bg-white p-10 rounded-xl border border-gray-200 text-center mt-6 shadow-sm">
                    ...
                    <p class="text-gray-500">Anda belum mendaftar kursus apapun.</p>
                </div>
            @endif
        </div>
            ...
            @if(isset($completedCourses) && $completedCourses->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                    @foreach($completedCourses as $course)
                        <div class="relative bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md hover:border-emerald-500 transition-all group flex flex-col h-full">
                            <a href="{{ route('courses.show', $course->slug) }}" class="block flex flex-col h-full">
                                <div class="h-32 bg-gray-200 relative overflow-hidden">
                                    <img src="{{ $course->image }}" alt="{{ $course->title }}" class="w-full h-full object-cover">
                                    <div class="absolute inset-0 bg-emerald-900 bg-opacity-70 flex flex-col items-center justify-center">
                                        <svg class="w-10 h-10 text-white mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                        <span class="text-white font-bold tracking-wider drop-shadow-md">Telah Selesai</span>
                                    </div>
                                </div>
                                <div class="p-5 flex-1 flex flex-col">
                                    <h3 class="text-lg font-bold text-gray-900 mb-4 group-hover:text-emerald-700 transition-colors">{{ $course->title }}</h3>
                                    <div class="mt-auto">
                                        <div class="flex justify-between items-center text-sm">
                                            <span class="text-emerald-600 font-bold bg-emerald-50 px-3 py-1 rounded-full">Lulus 100%</span>
                                            <span class="text-gray-500 font-medium hover:text-emerald-700">Lihat Lagi &rarr;</span>
                                        </div>
                                    </div>
                                </div>
                            </a>
        ...
        <div x-show="activeTab === 'favorit'" x-cloak>
            @if(isset($favoriteCourses) && $favoriteCourses->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
